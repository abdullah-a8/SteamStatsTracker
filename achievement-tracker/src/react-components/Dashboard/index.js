// src/react-components/Dashboard/index.js
...
import settingLogo from "./Static/settingLogo.png";
import loadingIcon from "./Static/loadingSign.png";
...
class DashBoard extends React.Component {
    ...
    extractStats(data) {
        let completion = -1;
        if (data && data.achievements !== undefined) {
    ...
    async updatePercentages() {
        ...
        for (let i = 0; i < gameList.length; i++) {
            ...
            let completion = -1;
            try {
                const res = await getAchievementStats(game.gameId);
                ...
                console.log(`Error updating percentages for gameId ${game.gameId}:`, error);
                gameList.splice(i, 1);
                i--;
            }
    ...
    updateGames(data) {
        if (!data) {
            console.log('Data is undefined in updateGames.');
            return;
        }
        let gameList = [];
    ...
    showChatBox(e) {
        ...
        if (this.state.friendList.filter(i => i.name === friendName)[0].onPending) {
    ...
    async onSubmitFriendRequest(e) {
        ...
        for (let i = 0; i < this.state.friendList.length; i++) {
            if (this.state.friendList[i].name === this.state.addFriendName) {
                alert("Friend already exists in your friend list");
                this.setState({ addFriendName: "" });
                return;
            }
        }
    ...
    onClickGameRedirectAchivement(e) {
        ...
        const gameItem = this.state.game.find(i => i.gameName === gameName);
        if (gameItem) {
            const gameId = gameItem.gameId;
